...
AddEventHandler('esx_phone:onMessage', function(phoneNumber, message, position, anon, job, dispatchRequestId)
	if job == 'player' then
		TriggerEvent("pNotify:SendNotification", {text = 'Du fick precis ett meddelande', type = "success", queue = "test", timeout = 1000, layout = "bottomCenter"})
	else
	...
	if dispatchRequestId then
		...
		CurrentActionMsg         = job .. ' - Tryck ~INPUT_CONTEXT~ för att ta SOS larmet'
...
RegisterNUICallback('activate_gps', function(data)
	...
	TriggerEvent("pNotify:SendNotification", {text = 'Positionen är på din gps', type = "success", queue = "test", timeout = 1000, layout = "bottomCenter"})
end)
...
RegisterNUICallback('send', function(data)
	...
	TriggerEvent("pNotify:SendNotification", {text = 'Meddelandet skickades', type = "success", queue = "test", timeout = 1000, layout = "bottomCenter"})
end)
...
RegisterNUICallback('bank_transfer', function(data, cb)
	...
	if amount ~= nil then
		...
		TriggerEvent("pNotify:SendNotification", {text = 'Du kan inte swisha såhär lite', type = "success", queue = "test", timeout = 1000, layout = "bottomCenter"})
	end
end)
...
if IsDisabledControlJustReleased(0, Keys['F1']) then
    if not GUI.IsOpen then
      TriggerServerEvent('esx_phone:checkItem')
    end
end
